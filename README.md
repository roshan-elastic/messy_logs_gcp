...
Streams structured logs from a Cloud Run service, a Cloud Function, and a Cloud SQL database through Pub/Sub and Dataflow into an Elasticsearch `logs.ecs` data stream.
...
2. **Writes those events to a Cloud SQL PostgreSQL table** (`sessions`), then queries back the 5 most recent rows to show on the page.
3. Returns an HTML page showing what was logged, the `session_id`, and the live database rows.
4. Logs flow through Cloud Logging → Pub/Sub → Dataflow → `logs.ecs` and typically arrive in **60–90 seconds**.

Two types of log are captured per request:

| Source | Log type | What it contains |
|---|---|---|
| Cloud Function stdout | `run.googleapis.com/stdout` | Structured JSON events (`session_id`, `user_id`, `event`, severity) |
| Cloud SQL | `cloudsql.googleapis.com/postgres.log` | The actual SQL statements: `CREATE TABLE`, `INSERT`, `SELECT` |
...
Query the `logs.ecs` data stream for the last 5 minutes. Three log sources land in the same stream:

| Filter | What you'll find |
|---|---|
| `resource.labels.service_name: "hello-world"` | HTTP access logs from the Cloud Run app |
| `resource.labels.service_name: "log-generator"` AND `logName: *stdout*` | Structured JSON events from the Cloud Function |
| `resource.labels.service_name: "log-generator"` AND `logName: *requests*` | HTTP access log for each browser request to the function |
| `resource.type: "cloudsql_database"` | SQL statements logged by PostgreSQL (`log_statement=all`) |

For Cloud Function events, use `jsonPayload.session_id` (shown on the function's HTML page) to correlate all events from a single browser request across both Cloud Function logs and Cloud SQL logs.
...
```
wired-streams-gcp/
├── main.tf                   - all GCP resources (Cloud Run, Cloud Function, Cloud SQL, Pub/Sub, Dataflow, GCS, IAM)
...
├── functions/
│   └── log_generator/
│       ├── main.py           - HTTP Cloud Function: generates structured log events + writes to Cloud SQL
│       └── requirements.txt  - Python dependencies (functions-framework, pg8000)
└── udfs/
